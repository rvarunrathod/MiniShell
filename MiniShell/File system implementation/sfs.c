...
// Here is the Part 0: implement a driver
...
/* ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ */
    ...
    printf("\033[1;31mSFS::\033[1;34m%s# \033[0m",current_working_directory);
...
/* ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ */
    ...
    if(strcmp(tokens[0],"exit")==0) {
    	...
    	printf("SFS: command not found: \033[1;31m%s\033[0m \n", tokens[0]);
      	continue;
    }
...
// Here is Part 1: implementation Display

void display(char *fname){
    char itype;
    char buffer[1024];
    
    int blocks[3]; // SO for current directory there are 3 block will be present
    blocks[0] = stoi(_inode_table[CD_INODE_ENTRY].XX, 2);
    blocks[1] = stoi(_inode_table[CD_INODE_ENTRY].YY, 2);
    blocks[2] = stoi(_inode_table[CD_INODE_ENTRY].ZZ, 2);
    ...
    for(int i=0; i<3; i++)
    {
        ...
        {   
            if(_directory_entries[j].F == '1')
            {
                if(strcmp(_directory_entries[j].fname, fname) == 0){
                	int temp = stoi(_directory_entries[j].MMM, 3);
                    itype = _inode_table[temp].TT[0];
                    if(itype == 'D')
                    {
                    	isDir=1;
                        continue;
        ...
        if(dest_inode!=-1) break;
    }

    if(isDir == 1 && dest_inode==-1) {
    	printf("display error: %s is not a file.\n",fname);
    	return;
    }
    else if(dest_inode == -1){
        printf("display error: %s file not found.\n",fname);
        return;
    }
    else{
        blocks[0] = stoi(_inode_table[dest_inode].XX, 2);
        blocks[1] = stoi(_inode_table[dest_inode].YY, 2);
        blocks[2] = stoi(_inode_table[dest_inode].ZZ, 2);
...
// Here is Part 2: implement a create file command

void create(char *fname){
    char itype;
    int blocks[3];
    int empty_block_indx = -1, dir_entry_indx = -1;
    ...
    if(empty_block_indx == -1 || dir_entry_indx == -1){
        ...
        switch(empty_block_indx){
            case 0: itos(_inode_table[CD_INODE_ENTRY].XX,newBlock,2); break;
            case 1: itos(_inode_table[CD_INODE_ENTRY].YY,newBlock,2); break;
            case 2: itos(_inode_table[CD_INODE_ENTRY].ZZ,newBlock,2); break;
        }
        blocks[empty_block_indx] = newBlock;
    }
    else{
        newInode = getInode();
        if(newInode == -1){
    ...
    itos(new_dir_block[dir_entry_indx].MMM,newInode,3);
    ...
    while(i < 3){
        ...
        if(i == 3){
            ...
            printf("Data will be truncated!\n");
            printf("3072 bytes saved.\n");
            return;
        }
    }
}

// Here is Part 3: implement a remove file/directory command
